...
Pseudocode for minimization by gradient descent:
  ...
  - intiialize step size $\epsilon$ and convergence tolerance $\delta$ to small, positive numbers
...
\\
& \begin{cases}
\mathbf{x}^T \mathbf{x} > 1 \quad\rightarrow  \quad \frac{\partial L}{\partial \lambda} > 1 \quad \rightarrow \text{gradient ascent increases } \lambda \\
\mathbf{x}^T \mathbf{x} > 1 \quad\rightarrow  \quad \frac{\partial L}{\partial \lambda} > 1 \quad \rightarrow \text{gradient ascent increases } \lambda 
\end{cases}
\end{align}
$$